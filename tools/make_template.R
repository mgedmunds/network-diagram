...
readme_rows <- list(
  ...
  list(style = note_style,    text = ""),
  list(style = note_style,    text = '     IF(A2="","",IFERROR(TEXTJOIN("; ",TRUE,IFERROR(VLOOKUP(FILTER(case_contexts!$B$2:$B$2001,case_contexts!$A$2:$A$2001=A2,""),contexts!$A$2:$B$1001,2,0),"")),""))'),
  list(style = note_style,    text = ""),
  list(style = note_style,    text = "  3. Press plain Enter (NOT Ctrl+Shift+Enter). This is a standard Excel 365 dynamic array formula."),
  list(style = note_style,    text = "  4. Copy M2, select M3:M1001, paste."),
  list(style = note_style,    text = "  Requires Microsoft 365. Shows ALL context names linked to each case, semi-colon separated."),
  list(style = note_style,    text = ""),
